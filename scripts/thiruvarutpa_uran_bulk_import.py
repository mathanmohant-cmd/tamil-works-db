...
from thiruvarutpa_uran_metadata import (
    WORK_METADATA,
    COLLECTION_ID,
    FILE_INFO,
    VERSE_METADATA
...
class ThiruvarutpaUranImporter(BaseWorkImporter):
    """
    Imports Thiruvarutpa Uran Adigal Edition using 2-phase bulk COPY pattern.

    Hierarchy:
    - Collection 323: பக்தி இலக்கியம்
      - Work: திருவருட்பா - ஊரன் அடிகள் பதிப்பு
    ...
    def __init__(self, db_connection_string="postgresql://postgres:postgres@localhost/tamil_literature"):
        ...
        print(f"Initialized ThiruvarutpaUranImporter")
    ...
    def _create_work_from_metadata(self) -> int:
        ...
        try:
            ...
            print(f"  [ERROR] {e}")
            print(f"  To re-import, first delete: python scripts/delete_thiruvarutpa_uran.py")
            sys.exit(1)

    def parse_file(self, file_path: str):
        """Parse Thiruvarutpa Uran Adigal Edition file"""
        print("\n=== PHASE 1: Parsing Thiruvarutpa Uran Adigal into memory ===")
...
def main():
    ...
    try:
        importer = ThiruvarutpaUranImporter(db_connection)
