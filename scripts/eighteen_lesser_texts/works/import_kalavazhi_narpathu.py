...
# -*- coding: utf-8 -*-
"""
Atomic import for களவழி நாற்பது (Kalavazhi Narpathu)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_kalavazhi_narpathu.py [database_url]

    # Link to different collection
    python import_kalavazhi_narpathu.py [database_url] --collection-id 201 --position 6
...
class KalavazhiNarpathuImporter(BaseWorkImporter):
    """Atomic importer for Kalavazhi Narpathu"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['kalavazhi_narpathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Kalavazhi Narpathu')
    ...
    print("="*70)
    print("Kalavazhi Narpathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = KalavazhiNarpathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_kalavazhi_narpathu.py')
        importer.rollback()
        sys.exit(1)
