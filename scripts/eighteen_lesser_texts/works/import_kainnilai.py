...
# -*- coding: utf-8 -*-
"""
Atomic import for கைந்நிலை (Kainnilai)
Thinai structure: Verses organized by thinai (landscape) sections

Usage:
    # Standalone (links to default collection 201)
    python import_kainnilai.py [database_url]

    # Link to different collection
    python import_kainnilai.py [database_url] --collection-id 201 --position 18
...
class KainnilaiImporter(BaseWorkImporter):
    """Atomic importer for Kainnilai"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['kainnilai'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Kainnilai')
    ...
    print("="*70)
    print("Kainnilai Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = KainnilaiImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_kainnilai.py')
        importer.rollback()
        sys.exit(1)
