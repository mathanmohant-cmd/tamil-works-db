...
# -*- coding: utf-8 -*-
"""
Atomic import for இனியவை நாற்பது (Iniyavai Narpathu)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_iniyavai_narpathu.py [database_url]

    # Link to different collection
    python import_iniyavai_narpathu.py [database_url] --collection-id 201 --position 4
...
class IniyavaiNarpathuImporter(BaseWorkImporter):
    """Atomic importer for Iniyavai Narpathu"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['iniyavai_narpathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Iniyavai Narpathu')
    ...
    print("="*70)
    print("Iniyavai Narpathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = IniyavaiNarpathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_iniyavai_narpathu.py')
        importer.rollback()
        sys.exit(1)
