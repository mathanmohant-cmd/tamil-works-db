...
# -*- coding: utf-8 -*-
"""
Atomic import for ஏலாதி (Elathi)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_elathi.py [database_url]

    # Link to different collection
    python import_elathi.py [database_url] --collection-id 201 --position 17
...
class ElathiImporter(BaseWorkImporter):
    """Atomic importer for Elathi"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['elathi'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Elathi')
    ...
    print("="*70)
    print("Elathi Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = ElathiImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_elathi.py')
        importer.rollback()
        sys.exit(1)
