...
# -*- coding: utf-8 -*-
"""
Atomic import for திணைமொழி ஐம்பது (Thinaymozhi Aimbathu)
Thinai structure: Verses organized by thinai (landscape) sections

Usage:
    # Standalone (links to default collection 201)
    python import_thinaymozhi_aimbathu.py [database_url]

    # Link to different collection
    python import_thinaymozhi_aimbathu.py [database_url] --collection-id 201 --position 9
...
class ThinaymozhiAimbathuImporter(BaseWorkImporter):
    """Atomic importer for Thinaymozhi Aimbathu"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['thinaymozhi_aimbathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Thinaymozhi Aimbathu')
    ...
    print("="*70)
    print("Thinaymozhi Aimbathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = ThinaymozhiAimbathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_thinaymozhi_aimbathu.py')
        importer.rollback()
        sys.exit(1)
