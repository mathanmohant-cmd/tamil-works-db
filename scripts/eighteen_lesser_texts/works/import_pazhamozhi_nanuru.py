...
# -*- coding: utf-8 -*-
"""
Atomic import for பழமொழி நானூறு (Pazhamozhi Nanuru)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_pazhamozhi_nanuru.py [database_url]

    # Link to different collection
    python import_pazhamozhi_nanuru.py [database_url] --collection-id 201 --position 14
...
class PazhamozhiNanuruImporter(BaseWorkImporter):
    """Atomic importer for Pazhamozhi Nanuru"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['pazhamozhi_nanuru'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Pazhamozhi Nanuru')
    ...
    print("="*70)
    print("Pazhamozhi Nanuru Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = PazhamozhiNanuruImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_pazhamozhi_nanuru.py')
        importer.rollback()
        sys.exit(1)
