...
# -*- coding: utf-8 -*-
"""
Atomic import for சிறுபஞ்சமூலம் (Sirupanchamoolam)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_sirupanchamoolam.py [database_url]

    # Link to different collection
    python import_sirupanchamoolam.py [database_url] --collection-id 201 --position 15
...
class SirupanchamoolamImporter(BaseWorkImporter):
    """Atomic importer for Sirupanchamoolam"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['sirupanchamoolam'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Sirupanchamoolam')
    ...
    print("="*70)
    print("Sirupanchamoolam Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = SirupanchamoolamImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_sirupanchamoolam.py')
        importer.rollback()
        sys.exit(1)
