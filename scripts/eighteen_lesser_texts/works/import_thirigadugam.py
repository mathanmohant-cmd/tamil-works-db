...
# -*- coding: utf-8 -*-
"""
Atomic import for திரிகடுகம் (Thirigadugam)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_thirigadugam.py [database_url]

    # Link to different collection
    python import_thirigadugam.py [database_url] --collection-id 201 --position 12
...
class ThirigadugamImporter(BaseWorkImporter):
    """Atomic importer for Thirigadugam"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['thirigadugam'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Thirigadugam')
    ...
    print("="*70)
    print("Thirigadugam Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = ThirigadugamImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_thirigadugam.py')
        importer.rollback()
        sys.exit(1)
