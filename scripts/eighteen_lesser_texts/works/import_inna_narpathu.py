...
# -*- coding: utf-8 -*-
"""
Atomic import for இன்னா நாற்பது (Inna Narpathu)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_inna_narpathu.py [database_url]

    # Link to different collection
    python import_inna_narpathu.py [database_url] --collection-id 201 --position 3
...
class InnaNarpathuImporter(BaseWorkImporter):
    """Atomic importer for Inna Narpathu"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['inna_narpathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Inna Narpathu')
    ...
    print("="*70)
    print("Inna Narpathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = InnaNarpathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_inna_narpathu.py')
        importer.rollback()
        sys.exit(1)
