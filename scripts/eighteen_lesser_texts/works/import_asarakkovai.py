...
# -*- coding: utf-8 -*-
"""
Atomic import for ஆசாரக்கோவை (Asarakkovai)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_asarakkovai.py [database_url]

    # Link to different collection
    python import_asarakkovai.py [database_url] --collection-id 201 --position 13
...
class AsarakkovaiImporter(BaseWorkImporter):
    """Atomic importer for Asarakkovai"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['asarakkovai'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Asarakkovai')
    ...
    print("="*70)
    print("Asarakkovai Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = AsarakkovaiImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_asarakkovai.py')
        importer.rollback()
        sys.exit(1)
