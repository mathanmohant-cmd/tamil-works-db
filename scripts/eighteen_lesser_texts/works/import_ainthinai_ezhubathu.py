...
# -*- coding: utf-8 -*-
"""
Atomic import for ஐந்திணை எழுபது (Ainthinai Ezhubathu)
Thinai structure: Verses organized by thinai (landscape) sections

Usage:
    # Standalone (links to default collection 201)
    python import_ainthinai_ezhubathu.py [database_url]

    # Link to different collection
    python import_ainthinai_ezhubathu.py [database_url] --collection-id 201 --position 8
...
class AinthinaiEzhubathuImporter(BaseWorkImporter):
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['ainthinai_ezhubathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Ainthinai Ezhubathu')
    ...
    print("="*70)
    print("Ainthinai Ezhubathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = AinthinaiEzhubathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_ainthinai_ezhubathu.py')
        importer.rollback()
        sys.exit(1)
