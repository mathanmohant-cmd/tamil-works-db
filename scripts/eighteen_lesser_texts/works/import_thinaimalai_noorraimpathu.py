...
# -*- coding: utf-8 -*-
"""
Atomic import for திணைமாலை நூற்றைம்பது (Thinaimalai Noorraimpathu)
Thinai structure: Verses organized by thinai (landscape) sections

Usage:
    # Standalone (links to default collection 201)
    python import_thinaimalai_noorraimpathu.py [database_url]

    # Link to different collection
    python import_thinaimalai_noorraimpathu.py [database_url] --collection-id 201 --position 10
...
class ThinaimalaiNoorraimpathuImporter(BaseWorkImporter):
    """Atomic importer for Thinaimalai Noorraimpathu"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['thinaimalai_noorraimpathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Thinaimalai Noorraimpathu')
    ...
    print("="*70)
    print("Thinaimalai Noorraimpathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = ThinaimalaiNoorraimpathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_thinaimalai_noorraimpathu.py')
        importer.rollback()
        sys.exit(1)
