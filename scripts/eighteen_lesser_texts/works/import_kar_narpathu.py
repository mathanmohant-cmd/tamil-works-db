...
# -*- coding: utf-8 -*-
"""
Atomic import for கார் நாற்பது (Kar Narpathu)
Flat structure: Single section, all verses directly under it

Usage:
    # Standalone (links to default collection 201)
    python import_kar_narpathu.py [database_url]

    # Link to different collection
    python import_kar_narpathu.py [database_url] --collection-id 201 --position 5
...
class KarNarpathuImporter(BaseWorkImporter):
    """Atomic importer for Kar Narpathu"""
    ...
    def __init__(self, db_connection_string, collection_id=None, position=None):
        ...
        metadata = WORK_METADATA['kar_narpathu'].copy()
        if position:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import Kar Narpathu')
    ...
    print("="*70)
    print("Kar Narpathu Atomic Import")
    print("="*70)
    print(f"Database: {args.database_url[:50]}...")

    importer = KarNarpathuImporter(
        args.database_url,
    ...
    try:
        ...
        print(f'  python delete_kar_narpathu.py')
        importer.rollback()
        sys.exit(1)
