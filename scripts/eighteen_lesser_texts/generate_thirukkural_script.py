...
# -*- coding: utf-8 -*-
"""
Generator for Thirukkural adhikaram structure import script.
Creates import/delete pair for 3-level hierarchy (Paal→Iyal→Adhikaram→Kurals).
"""
...
def generate_thirukkural_import_script(work_key, metadata):
    """Generate import script for Thirukkural (3-level adhikaram structure)"""
...
class {class_name}Importer(BaseWorkImporter):
        ...
        for line in lines_text:
            ...
            if line.startswith('$'):
                ...
                if current_kural_num is not None and current_kural_lines:
                    self._add_kural(current_kural_num, current_kural_lines)
                    kural_count += 1
                    current_kural_lines = []
                    current_kural_num = None
            ...
            # Check for kural marker (#N)
            kural_match = re.match(r'^#(\\\\d+)$', line)
            if kural_match:
                # Save previous kural if exists
                if current_kural_num is not None and current_kural_lines:
                    self._add_kural(current_kural_num, current_kural_lines)
                ...
                current_kural_num = int(kural_match.group(1))
                current_kural_lines = []
                continue
    ...
    def _add_kural(self, kural_num, kural_lines):
        """Add kural to memory"""
...
def main():
    print("="*70)
    print("  Generating Thirukkural Script (3-level Adhikaram Structure)")
    print("="*70)

    works_dir = Path(__file__).parent / 'works'

    work_key = 'thirukkural'
    if work_key in WORK_METADATA:
        metadata = WORK_METADATA[work_key]

        # Generate import script
        import_script = generate_thirukkural_import_script(work_key, metadata)
    ...
    print(f"\n✓ Generated Thirukkural 3-level adhikaram structure script pair (2 scripts)")
