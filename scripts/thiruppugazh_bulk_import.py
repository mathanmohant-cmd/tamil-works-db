...
from thiruppugazh_metadata import (
    WORK_METADATA,
    COLLECTION_ID,
    VERSE_METADATA,
    FILE_INFO
...
class ThiruppugazhBulkImporter(BaseWorkImporter):
    """
    Imports Thiruppugazh using 2-phase bulk COPY pattern.

    Hierarchy:
    - Collection 323: பக்தி இலக்கியம்
      - Work: திருப்புகழ்
        - Sections: @N markers (தொகுதி - volumes)
          - Verses: #N markers
    ...
    def __init__(self, db_connection_string="postgresql://postgres:postgres@localhost/tamil_literature"):
        ...
        print(f"Initialized ThiruppugazhBulkImporter")
    ...
    def _create_work_from_metadata(self) -> int:
        ...
        try:
            ...
            print(f"  [ERROR] {e}")
            print(f"  To re-import, first delete: python scripts/delete_thiruppugazh.py")
            sys.exit(1)
